...
# Center the window using win32api.
...
window_locked = False  # Used for both window locking and disabling resizing.
...
def on_press(key):
    ...
    # If the window is locked, disable resizing via F7/F8.
    if key in [keyboard.Key.f8, keyboard.Key.f7]:
        if window_locked:
            return
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                update_image()  # Immediately update the displayed image after resizing.
